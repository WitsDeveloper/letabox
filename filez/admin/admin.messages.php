...
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li>Messages</li></ol>
...
for ($i = 0; $i < $count; $i++){
	...
	?>	
	<td><a href="admin.php?tab=4&task=view&recid=<?=$i ?>"><?=$row["Subject"]?></a></td>
	<td><?=$row["Type"]?></td>
	<td><?=$row['Source']?></td>
	<td><a href="admin.php?tab=4&task=view&recid=<?=$i ?>">View</a> | <a href="admin.php?tab=4&task=del&recid=<?=$i ?>">Delete</a></td>
	</tr>
	<?php
}
db_free_result($res);
...
$sqlUpdateMsg = sprintf("UPDATE `".DB_PREFIX."messages` SET `DateRead` = '%s' WHERE `MsgID` = %d AND `ToAdd` = '%s'", $DateRead, $row["MsgID"], $MyEmail);
//Run query
...
echo list_attachments($row['MsgID']);
?>
</td>
<tr>
...
<div class="btn-group">
<a class="btn btn-default" href="admin.php?tab=4&amp;task=reply&amp;recid=<?=$recid?>">Reply</a>
<a class="btn btn-default" href="admin.php?tab=4&amp;task=replyall&amp;recid=<?=$recid?>">Reply All</a>
<a class="btn btn-default" href="admin.php?tab=4&amp;task=del&amp;recid=<?=$recid?>">Delete</a>
</div>
</td>
</tr>
</table>
...
<a class="btn btn-primary" href="admin.php?tab=4&task=add">Add Message</a>
...
<option<?=$folder1?> value="admin.php?tab=4&amp;folder=inbox">Inbox</option>
<option<?=$folder2?> value="admin.php?tab=4&amp;folder=outbox">Outbox</option>
</select>
</div>
</form>
</div>
...
<div class="quick-nav btn-group">
<a class="btn btn-default" href="admin.php?tab=4"><i class="fa fa-undo fa-fw"></i> Back to Messages</a>
<?php if ($recid > 0) { ?>
...
function viewrec($recid){
	...
	$row = db_fetch_array($res); 
	?>
	<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=4">Messages</a></li><li>View Message</li></ol>
	<?php 
			...
			if(sizeof($FIELDS['ToEmails']) > 0){
				...
				if(sql_insert($FIELDS)){
					...
					if(!$mail->Send()) {
						...
						redirect("admin.php?tab=4&recid=$recid");
					}else{
						$sent = true;
...
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=4">Messages</a></li><li class="active">Add Message</li></ol>

<a class="btn btn-default" href="admin.php?tab=4"><i class="fa fa-undo fa-fw"></i> Back to Messages</a>

<p class="text-center"><?php if(sizeof($ERRORS['MSG'])>0) echo $ERRORS['MSG'];?></p>
<form class="form-horizontal" id="validateform" enctype="multipart/form-data" action="admin.php?tab=4&task=add" method="post">
...
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=4">Messages</a></li><li class="active">Read Message</li></ol>

<a class="btn btn-default" href="admin.php?tab=4&task=view&recid=<?=$recid?>"><i class="fa fa-undo fa-fw"></i> Back to Messages</a>

<form class="form-horizontal" id="validateform" enctype="multipart/form-data" action="admin.php?tab=4&task=<?=$action?>&recid=<?=$recid?>" method="post">
<p class="text-center"><?php if(sizeof($ERRORS['MSG'])>0) echo $ERRORS['MSG'];?></p>
<input type="hidden" name="sql" value="update">
<input type="hidden" name="eid" value="<?=$row["MsgID"] ?>">
...
function deleterec($recid){
	...
	$row = db_fetch_array($res);  
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=4">Messages</a></li><li class="active">Delete Message</li></ol>
...
function sql_select(){
	...
	if($folder == "outbox"){
		$sql = sprintf("SELECT `MsgID`,`FromAdd`,`ToAdd`,`CcAdd`,`BccAdd`,`Subject`,`Message`,`DateSent`,`DateRead`,`Type`,`Source` FROM `".DB_PREFIX."messages` WHERE `FromAdd` = '%s'", $SysEmail);
		$res = db_query($sql,DB_NAME,$conn);
	}else{		
		$sql = "SELECT `MsgID`,`FromAdd`,`ToAdd`,`CcAdd`,`BccAdd`,`Subject`,`Message`,`DateSent`,`DateRead`,`Type`,`Source` FROM `".DB_PREFIX."messages` WHERE (`ToAdd` = '$SysEmail' OR `CcAdd` = '$SysEmail' OR `BccAdd` = '$SysEmail')";
		$res = db_query($sql,DB_NAME,$conn);
	}	
	return $res;
}
