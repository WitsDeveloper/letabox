   ...
   $product_code=$_POST["product_code"];
	foreach($_POST as $key => $value){
		$new_product[$key] = filter_var($value, FILTER_SANITIZE_STRING); 
          ...
          $new_product["price"] = $_POST['sellCost'];
...
$statement = $mysqli->prepare($query);
$statement->bind_param('issssssssssssssisss', $null,$Fname,$null,$null,$null,$Address1,$null,$null,$null,$null,$Phone,$Email,$encryptedPassword,$regDate,$null,$null,$null,$nullint,$com_code) or die('Error : ('. $mysqli->errno .') '. $mysqli->error);
$statement->execute();
...
<!--
    ...
    <!-- <button type="button" class="button small-btn yob-btn quickViewModal" data-id="<?php print $MediumImageURL.'!#@'.round($ConvertedSellingCost, 2).'!#@'.$ASIN.'!#@'.$Title.'!#@'.$DetailPageURL ?>" data-toggle="modal" data-target="#qView">quick view</button>-->
            ...
            <section class="name">
                <h3><a href="single" class="h3_in"><?php if( !empty($Title2) ){
                                          ...
                                          $CostUSD = $_POST["CostUSD"]; 
                                           $SellingCost = $_POST["SellingCost"]; 
                                            ...
                                            $OrderTotal = $total; 
                                              //$Status= $_POST["Status"]; 
                                             $Status=0; 
                                              if(isset($_POST["Order_comments"])){$Order_comments= $_POST["Order_comments"]; }
                                              else{
                                               $Order_comments='No comments';   
                                              }
                                              
$query2 = "INSERT INTO  `lbs_order` (lbs_bill_shipping_id,shipping_cost,Status,OrderTotal,weight,sellCostTotal,orderRemarks,OrderDate,processedBy,processeDate)"
        . " VALUES(?,?,?,?,?,?,?,?,?,?)";
$statement2 = $mysqli->prepare($query2);
$statement2->bind_param('isssssssss', $lbs_bill_shipping_id,$shipAmount,$Status,$OrderTotal,$weight,$sellCostTotal,$Order_comments,$regDate,$null_int,$null_date);
//$statement2->execute();
...
$margin = $product['margin'];
  ...
  $querymargins = "INSERT INTO  `lbs_order_margins` (OrderId,totalCost_Usd,totalCost_Ksh,convertedSellingcost_Ksh,convertedSellingcost_Usd,profitKes,profitUsd,margin,lbs_order_margins_date)"
        . " VALUES(?,?,?,?,?,?,?,?,?)";
$statement2mar = $mysqli->prepare($querymargins);
$statement2mar->bind_param('issssssss', $orderId,$totalCost_Usd,$totalCost_Ksh,$convertedSellingcost_Ksh,$convertedSellingcost_Usd,$profitKes,$profitUsd,$margin,$regDate);
//$statement2->execute();
