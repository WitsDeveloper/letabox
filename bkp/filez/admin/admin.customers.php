...
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li class="active">Customers</li></ol>
...
<td><?=$row["Fname"]?></td>
<td><?=$row["Phone"]?></td>
<td><?="<a href=\"admin.php?tab=4&amp;task=add&amp;email=".$row['Email']."\" title=\"Send Email\"><img border=\"0\" src=\"".IMAGE_FOLDER."/icons/mail.png\" height=\"22\" width=\"22\" alt=\"Send\"></a>";?></td>
<?php
if($row['disabledFlag'] == 0){
	echo "<td align=\"center\"><a href=\"admin.php?tab=5&enable=1&eid=".$row['UID']."\" title=\"Click to disable ".$row['CustomerName']."\"><img border=\"0\" src=\"".IMAGE_FOLDER."/icons/yes.png\" height=\"12\" width=\"12\" alt=\"Disable ".$row['CustomerName']."\"></a></td>";
}else{
	echo "<td align=\"center\"><a href=\"admin.php?tab=5&enable=0&eid=".$row['UID']."\" title=\"Click to enable ".$row['CustomerName']."\"><img border=\"0\" src=\"".IMAGE_FOLDER."/icons/no.png\" height=\"12\" width=\"12\" alt=\"Enable ".$row['CustomerName']."\"></a></td>";
}
?>
<td><?php if( isSuperAdmin() || isSystemAdmin() ) { ?><a href="admin.php?tab=5&task=view&recid=<?=$i ?>&customerID=<?=$row['lbs_bill_shipping_id'] ?>">Manage</a> | <a href="admin.php?tab=5&task=edit&recid=<?=$i ?>&customerID=<?=$row['lbs_bill_shipping_id'] ?>">Edit</a> 

| <a href="admin.php?tab=5&task=del&recid=<?=$i ?>&customerID=<?=$row['lbs_bill_shipping_id'] ?>">Delete</a>
<?php } else {?>N/A
<?php } ?>
</td>
</tr>        
    ...
    <div id="sub-tabs-1" class="tab-pane active">
      ...
      <div class="row">
        <div class="col-md-6">
          ...
          ?></td></tr>        
          <tr><td><strong>Email:</strong> </td><td><a href="admin.php?tab=4&task=add&email=<?=$row["Email"]; ?>" title="Send Email"><?=$row["Email"]; ?></a></td></tr>
...
print '<li> Order ID# '.$obj_ona1->OrderId.'</li>'; 
    ...
    <div class="quick-nav btn-group">
      <a class="btn btn-default" href="admin.php?tab=5&task=add"><i class="fa fa-file-o fa-fw"></i>Add Customer</a>
      <a class="btn btn-default" href="admin.php?tab=5&task=edit&recid=<?=$recid ?>&customerID=<?=$row['lbs_bill_shipping_id'] ?>"><i class="fa fa-pencil-square-o fa-fw"></i>Edit Customer</a>
      <a class="btn btn-default" href="admin.php?tab=5&task=del&recid=<?=$recid ?>&customerID=<?=$row['lbs_bill_shipping_id'] ?>"><i class="fa fa-trash-o fa-fw"></i>Delete Customer</a>
    </div>
...
function viewrec($recid){
	...
	?>
	<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=5">Customers</a></li><li class="active">View Customer</li></ol>
	<?php 
...
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=5">Customers</a></li><li class="active">Add Customer</li></ol>
...
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=5">Customers</a></li><li class="active">Edit Customer</li></ol>
...
function deleterec($recid){
	if( isSuperAdmin() || isSystemAdmin() ) {
		...
		$row = db_fetch_array($res);  
		?>
		<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=5">Customers</a></li><li class="active">Delete Customer</li></ol>
...
function manage_announcements($UserType){
	...
	switch ($action) {
		...
		case "edit":
			...
			?>
            <ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=5">Customers</a></li><li><a href="admin.php?tab=5#tabs-2">Announcements</a></li><li class="active"><?=ucwords($action)?> Announcements</li></ol>
            <p class="text-center"><?php if(sizeof($ERRORS['MSG'])>0) echo $ERRORS['MSG'];?></p>
            ...
            <tr>
            <td style="text-align:center" colspan="2"><input class="btn btn-primary" type="submit" name="Save" value="Save" /> <input class="btn btn-default" type="button" name="cancel" value="Cancel" onclick="javascript:location.href='admin.php?tab=5#tabs-2'" /></td>
            </tr>
            </table>
            </form>
            <?php
		...
		<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=5">Customers</a></li><li class="active">Announcements</li></ol>
...
function show_loginhistory(){
	...
	if(isset($_POST['DELETE']) && isset($_POST['logsIDs']) && isSuperAdmin()){	
		foreach($_POST['logsIDs'] as $selectedID){
			$sqlDelete = sprintf("DELETE FROM `".DB_PREFIX."customers_logs` WHERE `LogID` = %d LIMIT %d", intval($selectedID), 1);
			//Run query
			db_query($sqlDelete,DB_NAME,$conn);
		}
	...
	if(!empty($LoginID)){
		...
		$resSql = sprintf("SELECT `PL`.`LogID`, `PL`.`LoginID`, `PL`.`LoginDate`, `PL`.`Source` FROM `".DB_PREFIX."portal_logs` AS `PL` INNER JOIN `".DB_PREFIX."portal` AS `P` ON `PL`.`LoginID` = `P`.`LoginID` WHERE  `P`.`UserType` = 'Customer' AND `PL`.`LoginID` = '%s' ORDER BY `LoginDate` DESC LIMIT %d;", $LoginID, 10);
            ...
            //global $mysqli;
            $result = $mysqli->query("SELECT * FROM lbs_customers_logs") or die('Error : (' . $mysqli->errno . ') ' . $mysqli->error);
	...
	*/}		
	...
	<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?tab=5">Customers</a></li><li class="active">Login History</li></ol>
    ...
    <?php
     ...
     echo "<tr>
		  <td>".$row_log['UserID']."</td>
		  <td>".fixdatetime($row_log['LoginDate'])."</td>
		  <td>".$row_log['Source']."</td>                  
