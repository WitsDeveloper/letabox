...
while($obj_ptds= $pdts->fetch_object())
     {
    print '<tr class="ona_order"><td>'.$obj_ptds->ProductName.'X <b>'.$obj_ptds->ProductQty.'</b></td><td>'.wits_money($obj_ptds->sellCost).'</td></tr>'; 
    
    $orderSum += $obj_ptds->sellCost;
     }
     ?>
     <tr><td>Shipping:</td> <td>Kes <?php print $shippingCost; ?></td> </tr>    
          <tr><td>Payment method:</td> <td>Cash Payment</td> </tr>
          <tr><td><b>Total:</b></td> <td>Kes <?php print wits_money($orderSum);?></td> </tr>  
</table>
