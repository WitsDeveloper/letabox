...
//order state
        global $mysqli;
$project_desc1a = $mysqli->query("SELECT * from lbs_order WHERE OrderId=".$row["OrderId"])or die('Error : ('. $mysqli->errno .') '. $mysqli->error);     
$obj_ona1a= $project_desc1a->fetch_object();
$OrderStatus=$obj_ona1a->Status;
?>
...
?>
    <a href="admin.php?tab=3&OrderStatus=<?php print $OrderStatus; ?>&task=process&OrderId=<?php print $row['OrderId'];?>"><?php print $row['OrderId'].' '.$Fname=$obj_ona1->Fname; ?> <i  style="font-weight:bold;"class="fa fa-eye" aria-hidden="true"></i></a>
    </td>
<td><?php
...
?>
          ...
          <!--purchased-->
           <?php  otherOrders($orderState=1); ?> 
          </div>
          <div id="tabs-3" class="tab-pane">
           <!--purchased-->
           <?php  otherOrders($orderState=7); ?> 
            <!--End Forms-->
          </div>
        <div id="tabs-4" class="tab-pane">
           <!--purchased-->
           <?php  otherOrders($orderState=8); ?> 
            <!--End Forms-->
          </div>
     <div id="tabs-5" class="tab-pane">
           <!--purchased-->
           <?php  otherOrders($orderState=0); ?> 
            <!--End Forms-->
          </div>
...
function sql_select_purchased(){
	global $conn;		
	$sql = "SELECT * FROM lbs_order WHERE Status='1' ORDER BY OrderId DESC";	
        $res = db_query($sql,DB_NAME,$conn);
	return $res;
}
...
function sql_getrecordcount_purchased(){
	global $conn;
	
	$sql = "SELECT COUNT(*) FROM lbs_order WHERE Status='1'";	
	$res = db_query($sql,DB_NAME,$conn);
	$row = db_fetch_array($res);
	reset($row);
	return current($row);
}
function otherOrders($orderState){
      showpagenav($pagecount); ?>
        <table width="100%" class="display table table-striped table-bordered table-hover">
<thead>
<tr>
...
$productID=$obj_ona21->ProductId;
 ...
 <?php  print '</td><td>'.$Title.'X <b>'.$obj_ptds->ProductQty.'</b></td><td>'.wits_money($ConvertedSellingCost).'</td></tr>';  
    ...
    <div class="<?php if($OrderStatus<1) { ?>col-md-4 <?php } else{ ?>col-md-6 <?php } ?>">
...
$(document).ready(function(){
 $(".manthaOrderId").submit(function(e){
     e.preventDefault();
  var rowid = $("#orderId").val(); 
     $.ajax({
            type : 'html',
            url : 'new_process.php', 
            data :  'rowid='+ rowid, 
