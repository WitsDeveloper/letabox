...
if($statement->execute()){
 ...
 print '<script type="text/javascript">'.'alert("Settings saved.");'.'</script>';     
  ...
  //MySqli Select Query
  if(isset($_POST['manthaOrder'])) {
     global $mysqli;
  $orderId = $_POST['rowid'];
$project_desc1 = $mysqli->query("SELECT * from lbs_order WHERE OrderId='$orderId'")or die('Error : ('. $mysqli->errno .') '. $mysqli->error);     
while($obj_ona1= $project_desc1->fetch_object())
     {
//print 'hapa->'.$obj_ona1->lbs_bill_shipping_id; 
?>
<div id="proc_resp"  class="col-md-12">
   <?php print 'hapa->'.$obj_ona1->lbs_bill_shipping_id; ?> 
    
    dgdgdgdgdgdgdgdgdgdgddgdgdgdgdgdgdgddgdgdgdgdgdgdgdgdgdgdgdgd
</div>
