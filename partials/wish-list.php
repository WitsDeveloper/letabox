...
while($loop < $variation_width){  
	...
	echo '<select class="variationzz" name="'.$data['VariationDimensions']['VariationDimension'][$loop].'">';
		foreach($data['Item'] as $i ):
			//print_r($option_v = $i['VariationAttributes']['VariationAttribute']);
			$option_value = $i['VariationAttributes']['VariationAttribute'][$loop]['Value'];
			echo '<option value="'.$i['ASIN'].','.$option_value.'">'.$option_value.'</option>';
