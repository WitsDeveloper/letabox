...
include 'includes/mysqli.functions.php';
//get IP ADDRESS
...
if(isset($_POST["product_code"]))
{
  ...
  $current_variations = explode(",",$_POST['variations']);
  foreach( $current_variations as $cv ):      
      $keycv = $_POST[$cv];
      if(isset($keycv)){
       //array_push($attr, $cv.': '.$_POST[$cv]);
       $attr[$product_code]['variations'][$cv] .= $keycv;
      }
   endforeach;
 // $attr = array();
  //$arr=explode(",",$str);
   //print_r($arr);
  /*foreach( $current_variations as $key=>$cv ):      
       if(isset($_POST[$cv])){
       array_push($attr, $cv.': '.$_POST[$cv]);
...
$_SESSION["productsx"][$new_product['product_code']] = $new_product;	
                ...
                //end the new cart update	
	
 	//$total_items = count($_SESSION["productsx"]); 
	//die(json_encode(array('items'=>$total_items,'product_code'=>$product_code,'attr'=>$attr))); 
        	die(json_encode(array('items'=>$total_items,'product_code'=>$product_code)));
   //$var_info = print_r($attr,true);

//$var_info = print_r($new_product,true);
  ...
  if( $response["Items"]["Request"]["IsValid"] ) {    
      if( is_array( $response["Items"]["Item"] ) ) {
          ...
          $DetailPageURL = $response["Items"]["Item"]["DetailPageURL"];
                ...
                $EditorialReviewContent_x=truncate($EditorialReviewContent2, 240);
            }
                     ...
                     /*$optionz = explode(",", $optionz);
   $ASIN=$optionz[0];
   $VALUE=$optionz[1];
   $varName=$optionz[2];
     echo json_encode(array('asin'=>$ASIN,'valuez'=>$VALUE,'var_name'=>$varName));
    * */
   //echo json_encode($ASIN);
 
   $var.= '<ul>';
   foreach (array_slice($response["Items"]["Item"]["ImageSets"]["ImageSet"], 0, 4) as $value) {                         
                            $visa = $value["LargeImage"]["URL"];
                            if (!empty($value)) {    
   $var.='<li><a href="'.$visa.'"><img src="'.$visa.'" alt=""> </a></li>';
                                      }
                                    }
    $var.= '</ul>';   
    $var.='<img src="'.$LargeImageURL.'" alt="" id="mainGall"><span class="clearfix"></span>';
   }
      }      
       //$var;  
$cartInfo.='<input name="totalCost_Usd" type="hidden" value="'.$TotalCost.'">';
$cartInfo.='<input name="totalCost_Ksh" type="hidden" value="'.$ConvertedTotalCost.'">';
$cartInfo.='<input name="convertedSellingcost_Ksh" type="hidden" value="'.$ConvertedSellingCost.'">';
$cartInfo.='<input name="convertedSellingcost_Usd" type="hidden" value="'.$ConvertedTotalCost_Usd.'">';
$cartInfo.='<input name="profitKes" type="hidden" value="'.$profitKes.'">';
$cartInfo.='<input name="profitUsd" type="hidden" value="'.$profitUsd.'">';
$cartInfo.='<input name="margin" type="hidden" value="'.$marginPercentage.'">';
$cartInfo.='<input name="price" type="hidden" value="'.$Amount.'">';
$cartInfo.='<input name="sellCost" type="hidden" value="'.round($ConvertedSellingCost, 2).'">' ; 
$cartInfo.='<input name="title" type="hidden" value="'.$Title.'">'; 
$cartInfo.='<input name="quantity" type="hidden" value="1" placeholder="Quantity">';
$cartInfo.='<input name="product_code" type="hidden" value="'.$ASIN.'">';
$cartInfo.='<input name="thumbnail" type="hidden" value="'.$MediumImageURL2.'">';                 
$cartInfo.='<input name="product_code" type="hidden" value="'.$ASIN.'">';  
$cartInfo.='<input name="product_url" type="hidden" value="'.$DetailPageURL.'">';  
$cartInfo.='<input type="hidden" name="weight" value="'.$ConvertedWeight.'"/>';
$cartInfo.='<div class="nav_cart_qty">';
$cartInfo.='<div class="input-group"> ';   
$cartInfo.='<input type="text" data-code="'.$product_code.'" name="quantity" class="form-control input-number text-center quantity" value="1"/>';
$cartInfo.='<button type="button" id="add" data-code="nullz" class="add btn cart_btn"><img src="images/icons/plus.png"></button>';
$cartInfo.='<button type="button" id="sub2" value="-" field="quantity" data-code="nullz" class="sub2 btn cart_btn"><img src="images/icons/minus.png"></button>'; 
$cartInfo.='<input name="product_code" type="hidden" value="'.$ASIN.'">'; 
$cartInfo.='<input name="thumbnail" type="hidden" value="'.$MediumImageURL.'">';   
$cartInfo.='</div></div>';                                        
 print json_encode(array('carousl'=>$var,'price'=>$FormattedPrice,'stoNew'=>$TotalNew,'stoRef'=>$TotalRefurbished,'stoUsed'=>$TotalUsed,'orig_tit'=>$Title,'briefDes'=>$EditorialReviewContent_x,'Des'=>$EditorialReviewContent2,'formItem'=>$cartInfo)); 
